...
class ProductQuery:
	...
	def query_items(self, start=0):
		...
		strQuery = """
			SELECT  DISTINCT tabWebsiteItem.web_item_name, tabWebsiteItem.name, tabWebsiteItem.item_name, tabWebsiteItem.item_code, tabWebsiteItem.website_image,
					tabWebsiteItem.variant_of, tabWebsiteItem.has_variants, tabWebsiteItem.item_group, tabWebsiteItem.image, tabWebsiteItem.web_long_description,
					tabWebsiteItem.short_description, tabWebsiteItem.route, tabWebsiteItem.website_warehouse, tabWebsiteItem.ranking, tabWebsiteItem.on_backorder
					, tabBatch.expiry_date AS expiry_date, tabBatch.best_value_date AS best_value_date , tabBatch.name AS batch_no , tabBatch.ranking AS batch_ranking
					"""

		strFrom = """
				FROM `tabWebsite Item` AS tabWebsiteItem
				INNER JOIN `tabItem` ON tabWebsiteItem.item_code = tabItem.item_code
				INNER JOIN `tabBatch` ON tabBatch.item =  tabItem.name AND tabBatch.disabled = 0 AND (expiry_date > CURRENT_TIMESTAMP OR expiry_date = '' OR expiry_date is null)
				INNER JOIN `tabStock Ledger Entry` AS tabStockLedgerEntry ON tabStockLedgerEntry.item_code = tabItem.item_code
		...
		for row in filters[1:]:			
				if row:
					if len(row) == 4 :						
						strFrom += ' INNER JOIN `tab' + row[0] +'`  ON tabWebsiteItem.item_code = `tab' + row[0] +'`.parent '						
						strWhere += ' AND `tab' + row[0] +'`.' + row[1] + ' ' +  row[2] + ' ( ' + "'" + "' , '".join(value.replace("'", r"\'") for value in row[3]) + "'"  +' ) '						
					else:																											
						if row[1] == 'in':
							if 'best_value_page' in row[2]:
							...
							if row[2] != ['best_value_page'] and row[2] != ['pre_order_page']:  ###End Custom Update
								strWhere += ' AND tabWebsiteItem.' + row[0] + ' ' +  row[1] + ' ( ' + "'" + "' , '".join(value.replace("'", r"\'") for value in row[2]) + "'"  +' ) '				
						else:							
							strWhere += ' AND tabWebsiteItem.' + row[0] + ' ' +  row[1] + "'" +  str(row[2]) + "'"
		### Custom Update
		or_filters = self.or_filters
		if or_filters:
			strWhere += ' AND (tabWebsiteItem.' + or_filters[0][0] + ' ' + or_filters[0][1] + "'" +  or_filters[0][2] + "'" 
			for row in or_filters:
				if row:
					strWhere += ' OR tabWebsiteItem.' + row[0] + ' ' + row[1] + "'" +  row[2] + "'"
			strWhere += ')'
		strQuery += strFrom + strWhere	+ ' ORDER BY tabBatch.ranking desc, tabWebsiteItem.item_name '
		### End Custom Update		
		...
		return items[start:start + page_length], count
	...
	def add_display_details(self, result, discount_list, cart_items):
		...
		for item in result:	
			# custom update
			if item.batch_no:
				product_info = get_product_info_for_website(item.item_code , True , item.batch_no ).get('product_info')
			else:
				product_info = get_product_info_for_website(item.item_code, True ).get('product_info')
			...
			# custom update replace item_Code with batch_no
			item.in_cart = item.batch_no in cart_items
	...
	def get_stock_availability(self, item):
		...
		if not is_stock_item:
			if warehouse:
				...
				item.in_stock = True
		elif warehouse and item.batch_no:
			# stock item and has warehouse														
			actual_qty = get_batch_qty(item.batch_no, warehouse, item.item_code)
			item.in_stock = bool(flt(actual_qty))										
		elif not warehouse and item.batch_no:			
			actual_qty = 0
			warehouse_qty_list = get_batch_qty(item.batch_no)
	...
	def get_cart_items(self):
		...
		if customer:
			...
			if quotation:
				...
				# custom update replace item_Code with batch_no
				items = [row.batch_no for row in items]
				return items
