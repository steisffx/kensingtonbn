	...
	*/
	...
	prepare_toolbar() {
		this.products_section.append(`
			...
			<style>@keyframes loading {0%{color: #e38888;}10% {color: #e3dc88;}20% {color: #9ae388;}30% {color: #88e3c6;}40% {color: #F47A7A;}50% {color: #827af4;}60% {color: #a7a6b5;}70% {color: #da57fb;}80% {color: #b53a56;}90% {color: #f95c00;}100% {color: #efd51e;}}</style> 
			<div id="loadstart" class="container pt-sm-8 pt-md-32 pt-lg-38  pt-xl-44  col-6  " style="z-index:100;">
				<div class="d-flex flex-column justify-content-center align-items-center">
				<img  src="/assets/kensingtonbn/images/loading-cargando.gif"width="168" height="168">
				<br>
				<h3 class="text-center" style="color: #e38888;animation-name: loading;animation-duration: 6s;">LOADING</h3>
				</div>
			</div>
		`);
		this.prepare_search();
	...
	get_item_filter_data(from_filters=false) {
		...
		frappe.call({
			method: "erpnext.www.all-products.index.get_filters",
			args: {
				query_args: args
			},
			callback: function(r){
				if (r.message){  
					...
					$('.filter-options').each(function(){
						scrollbars[$(this).attr('data-filter')] = $(this).scrollTop()

					})  
					jQuery('#product-filters').html('');
					$(r.message).appendTo('#product-filters');
					me.bind_filters()
					me.restore_filters_state();
					$('.panel-collapse').each(function(){
						if (panels.includes($(this).attr('id'))){
							$(this).addClass('show')
						}
					}) 
					$('.filter-options').each(function(){
						if ($(this).attr('data-filter') in scrollbars){
							$(this).scrollTop(scrollbars[$(this).attr('data-filter')]);
						}
		...
		frappe.call({
			...
			callback: function(result) {
				if (!result || result.exc || !result.message || result.message.exc) {
					...
					//custom update
					var s = $("#placeholdersuggested");
					if ( s.length == 0){
						me.get_suggested_section();
					}
	...
	render_grid_view(items, settings) {
		...
		// custom update for showing loading before fetching items content
		...
		if (this.products) {
			let paging_html = `
				<div class="row product-paging-area mt-5">
					...
					<div class="col-9 text-right" style="display: flex;flex-direction:row;overflow-x:scroll;">
			`;
			...
			//custom update paging start
			paging_html += `
				<button class="btn btn-default btn-prev" data-start="${ start - page_length }"
					style="float: left; order:0;" ${prev_disable}>
					${ __("Prev") }
				</button>`;
				
				for(let startnum=1 ; startnum<=(parseInt((this.product_count/page_length)+1));startnum++){
					// console.log(startnum);
					// console.log((startnum-1)*12 );
					
					paging_html += `
					<button class="btn btn-default btn-current" data-start="${ (startnum-1)*12 }"
						style="float: left; order:${ startnum };" >
						${ startnum }
					</button>`;	
				}
			
			paging_html += `
				<button class="btn btn-default btn-next" style="order:${ parseInt((this.product_count/page_length))+2};" data-start="${ start + page_length }"
					${next_disable}>
					${ __("Next") }
				</button>
			`;
			//custom update paging end
			paging_html += `</div></div>`;
			///custom update paging active
			$(".page_content").append(paging_html);
			var path = window.location.search;
			const urlParams = new URLSearchParams(path);
			var SelectedPage = urlParams.get("start"); 
			if (SelectedPage){
				$('.btn-current[data-start="'+SelectedPage+'"]').addClass('active');
			}
			else{
				$('.btn-current[data-start="0"]').addClass('active');
			}
			///end custom update paging active
			this.bind_paging_action();
		}
	}
	//custom update
	get_suggested_section() {
		let me = this;
		frappe.call({
			method: 'kensingtonbn.www.pages.suggesteditems.get_items_html',
			callback: function(r) {
				if (!r.exc) {
					jQuery('#placeholdersuggested').html('');
		
					if(typeof r.message != 'undefined'){              
					me.add_suggested_section();
	...
	add_suggested_section() {
		let suggested_html = `<div class="row" id="placeholdersuggested"></div>`;
			$(".page_content").append(suggested_html);	
	}
	//custom update end
	prepare_search() {
		$(".toolbar").append(`
			<div class="input-group col-8 p-0">
				<div class="dropdown w-100" id="dropdownMenuSearch">
					...
					<input type="search" name="query" id="search-box" class="form-control font-md"
						placeholder="Search for Products" value="${window.location.search.includes('query')? new URLSearchParams(window.location.search).get("query") : '' }"
	...
	bind_paging_action() {
		let me = this;
		//custom update
		$('.btn-prev, .btn-next, .btn-current').click((e) => {
			const $btn = $(e.target);
	...
	bind_filters() {
		...
		$('.product-filter').on('change', (e) => {
			...
			//// Custom update
			$('.product-filter').each(function(){
				if ($(this).attr('data-filter-name') ==  $checkbox.data('filter-name') ){
