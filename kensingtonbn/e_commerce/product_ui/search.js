erpnext.ProductSearch = class {
	constructor() {
		...
		this.search_enter_listener(); ///custom
	}
	...
	////Custom update Enter Event Listener on Search field
	search_enter_listener(){
		this.searchBox.on("keyup", (e) => {
			if (e.keyCode === 13) {
				let query = e.target.value;
				if (!query.length) window.location.href;

				if ((!window.location.search) || (window.location.search.includes('query='))){
					let path = window.location.pathname + '?' + 'query=' + query;
	...
	bindSearchAction() {
		...
		this.searchBox.on("input", (e) => {
			...
			/// Custom Update
			var page = ""
			if (window.location.search.includes('best_value_page')){
				page = "best_value"
			}
			else if(window.location.search.includes('pre_order_page')){
				page = "pre_order"
			}
			/// End custom update
			frappe.call({
			...
			html += `
				<div class="dropdown-item" style="display: flex;">
					...
					<div class="col-9" style="white-space: normal;">
						<a href="/${res.route}${res.batch_no ?'?batch='+res.batch_no:''}">${res.web_item_name}</a><br>
