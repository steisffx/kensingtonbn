...
{% include 'kensingtonbn/public/js/controllers/buying.js' %};
...
frappe.ui.form.on("Purchase Order", {
	...
	refresh: function(frm) {
		...
		//alaa update
		// custom update to filter items by sku values
		console.log("ffffff")

	},
	//alaa
	//custom update to scan items by sku values in barcode
    scan_barcode: function(frm){
        let filter = frm.doc.scan_barcode;
        if (filter){
            frappe.call({
                method:"kensingtonbn.www.api.api.sku_item_code",
                args:{test:filter}
            }).done((r)=>{
            // frm.doc.items=[]
			// console.log(r.message)
                $.each(r.message,function(i,e){
					...
					frappe.model.set_value(entry.doctype, entry.name, "item_code", e.item_code);
...
frappe.ui.form.on("Purchase Order Item", {
	...
	//alaa
	//custom update to scan items by sku values in barcode inside child table
    // barcode: function(frm, cdt, cdn){
	// 	var row = locals[cdt][cdn];
    //     let filter = row.barcode;
    //     if (filter){
    //         frappe.call({
    //             method:"kensingtonbn.www.api.api.sku_item_code",
    //             args:{test:filter}
...
erpnext.buying.PurchaseOrderController = erpnext.buying.BuyingController.extend({
	...
	refresh: function(doc, cdt, cdn) {
		...
		// 		console.log("try222")
		// 		return {
		// 			// query: "kensingtonbn.www.api.api.item_query",
		// 			filters: {'item_code': "Kinder Surprise 20g"}
		// 		}
		// 	}
		// }, 500);
	},
