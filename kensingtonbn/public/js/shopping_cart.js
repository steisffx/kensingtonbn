...
var addcart_TimeOut =null; ///custom
...
frappe.ready(function() {
	...
	shopping_cart.bind_spiner_action();/////custom
});
...
$.extend(shopping_cart, {
	...
	update_cart: function(opts) {
		if (frappe.session.user==="Guest") {
			...
			clearTimeout(addcart_TimeOut);
			addcart_TimeOut = null;
			addcart_TimeOut = setTimeout(function(){ // // end custom update
				...
				return frappe.call({
					...
					args: {
						...
						qty: opts.qty,
						batch_no: opts.batch_no, //cutome update
	...
	set_cart_count: function(animate=false) {
		...
		// var $cart = $('.cart-icon');
		var $cart = $('.shopping-cart-icon'); ///custom
	...
	shopping_cart_update: function({item_code, qty,batch_no, cart_dropdown, additional_notes}) {
	...
	//custom
	bind_spiner_action(){
		$('.page_content').on('click', '.number-spinner-item button', function (e) {
			
			var btn = $(e.currentTarget),
				input = btn.closest('.number-spinner-item').find('input'),
			...
			if (newVal == 0) {
				...
				if(window.location.pathname == '/all-products'){
					...
					$('.btn-add-to-cart-list[data-batch = "'+batch_no+'"]').removeClass('hidden');	
					$('.btn-add-to-cart-list[data-batch = "'+batch_no+'"]').prop('disabled', false);
				}
				else{

					$('.number-spinner-item, .btn-add-to-cart').toggleClass('hidden');
	...
	bind_add_to_cart_action() {
		$('.page_content').on('click', '.btn-add-to-cart-list', (e) => {
			...
			// custom update
			$btn.parent().find('.number-spinner-item').removeClass('hidden');
			// $btn.parent().find('.go-to-cart').removeClass('hidden');
			// $btn.parent().find('.go-to-cart-grid').removeClass('hidden');
			// $btn.parent().find('.cart-indicator').removeClass('hidden');
			const item_code = $btn.data('item-code');
			const batch_no = $btn.data('batch'); // custom update			
