...
erpnext.PointOfSale.Payment = class {
	...
	bind_events() {
		...
		this.$component.on('click', '.submit-order-btn', () => {
			...
			{
				total_before_discount += doc.items[i].base_price_list_rate * doc.items[i].qty;
			}
			if (doc.discount_amount > 0) {
				let d = new frappe.ui.Dialog({
					...
					primary_action(values) {
						frappe.db.get_list("Apply Discount Passwords",{
							fields: ["password"]
						}).then((data) => {							
							for(let pass in data){																
								if (values.password == data[pass].password) {
									d.hide();
									me.events.submit_invoice();
								 	return;
								}														
							}	
							frappe.msgprint("Password Incorrect.");						
